      ...
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ padding: 24, marginBottom: 24 }}>
          ...
          <button type="submit" disabled={loading} className="btn btn-primary btn-lg w-full mt-6" style={{ fontSize: 16, fontWeight: 700 }}>
            {loading ? (
              <><span className="animate-spin inline-block mr-2">⚙️</span> AI Analyzing Berth Options...</>
            ) : (
              <>🧠 Get Berthing Recommendation</>
            )}
          </button>
        </div>
      </form>
      ...
      {results && best && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {[
              { icon: '🎯', value: `${best.confidence.toFixed(0)}%`, label: 'Top Confidence', color: confColor(best.confidence) },
              { icon: '⏱️', value: `${best.expected_wait_hours.toFixed(1)}h`, label: 'Avg. Wait (Historical)', color: '#0ea5e9' },
              { icon: '🔧', value: `${best.expected_service_hours.toFixed(0)}h`, label: 'Avg. Service (Historical)', color: '#8b5cf6' },
              { icon: '🏗️', value: `${results.length}`, label: 'Total Berths Ranked', color: '#f59e0b' },
            ].map((kpi, i) => (
                      ...
                      <div className="flex-1 relative" style={{ height: 40, background: 'rgba(0,0,0,0.03)', borderRadius: 8, border: '1px solid rgba(0,0,0,0.04)' }}>
                        ...
                        {opt.expected_wait_hours > 0 && (
                          <div style={{
                            ...
                            ⏳ {opt.expected_wait_hours.toFixed(1)}h
                          </div>
                        )}
                        {/* Service period */}
                        <div
            ...
            {hoveredBar && (
              <div style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 12px', fontSize: 11, marginBottom: 6 }}>
                  ...
                  <span>⏳ Wait: <strong>{hoveredBar.expected_wait_hours.toFixed(1)}h</strong></span>
                  <span>🔧 Service: <strong>{hoveredBar.expected_service_hours.toFixed(0)}h</strong></span>
                  <span>🎯 Suitability: <strong>{hoveredBar.suitability_score.toFixed(0)}%</strong></span>
                  <span>⚠️ Risk: <strong style={{ color: hoveredBar.risk_score > 20 ? '#ef4444' : '#10b981' }}>{hoveredBar.risk_score}%</strong></span>
                </div>
                ...
                <div style={{ padding: '20px 20px 16px' }}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    {opt.cons.slice(0, 2).map((c, j) => (
                      <div key={j} className="flex items-start gap-2 mb-1">
                        <span style={{ color: '#ef4444', fontSize: 11, marginTop: 2 }}>✗</span>
          ...
          >
            ...
            <span style={{ fontSize: 14, color: 'var(--color-text-muted)' }}>{breakdownOpen ? '▲' : '▼'}</span>
          </div>
          {breakdownOpen && (
            <div style={{
...
function buildBreakdown(form: VesselForm, berthLoa: number, berthDepth: number, berthBeam: number, cargoMatch: boolean, vesselTypeMatch: boolean, waitH: number, serviceH: number): ParameterCheck[] {
  ...
  checks.push({ category: 'Physical Fit', parameter: 'LOA', status: loaStatus, icon: loaStatus === 'fail' ? '❌' : loaStatus === 'tight' ? '⚠' : '✔', detail: `LOA (${form.loa}m) ${loaSlack >= 0 ? 'fits within' : 'EXCEEDS'} berth limit (${berthLoa}m) → ${loaSlack >= 0 ? `Safe margin: ${loaSlack}m` : `Over by ${Math.abs(loaSlack)}m`}`, compact: `LOA ${loaSlack >= 0 ? loaSlack + 'm margin' : 'OVER'}` });

  const draftStatus = draftSlack > 3 ? 'pass' : draftSlack > 1.5 ? 'ok' : draftSlack > 0 ? 'tight' : 'fail';
  checks.push({ category: 'Physical Fit', parameter: 'Draft', status: draftStatus, icon: draftStatus === 'fail' ? '❌' : draftStatus === 'tight' ? '⚠' : '✔', detail: `Draft (${form.draft}m) vs berth depth (${berthDepth}m) → UKC: ${draftSlack.toFixed(1)}m ${draftSlack < 1.5 ? '(below 1.5m minimum)' : ''}`, compact: `UKC ${draftSlack.toFixed(1)}m` });

  const beamStatus = beamSlack > 10 ? 'pass' : beamSlack > 3 ? 'ok' : beamSlack > 0 ? 'tight' : 'fail';
  checks.push({ category: 'Physical Fit', parameter: 'Beam', status: beamStatus, icon: beamStatus === 'fail' ? '❌' : beamStatus === 'tight' ? '⚠' : '✔', detail: `Beam (${form.beam}m) vs berth max beam (${berthBeam}m) → Clearance: ${beamSlack}m`, compact: `Beam ${beamSlack}m clear` });

  // Operational
  const cargoStatus = cargoMatch ? 'pass' : 'tight';
  checks.push({ category: 'Operational', parameter: 'Cargo Type', status: cargoStatus, icon: cargoMatch ? '✔' : '⚠', detail: cargoMatch ? `${form.cargo_type} is a primary cargo type handled at this berth` : `${form.cargo_type} requires equipment adaptation at this berth`, compact: cargoMatch ? 'Cargo matched' : 'Cargo needs adaptation' });

  const vtStatus = vesselTypeMatch ? 'pass' : 'tight';
  checks.push({ category: 'Operational', parameter: 'Vessel Type', status: vtStatus, icon: vesselTypeMatch ? '✔' : '⚠', detail: vesselTypeMatch ? `${form.vessel_type} is an allowed vessel type` : `${form.vessel_type} not primary type — may require operational adjustments`, compact: vesselTypeMatch ? 'Type matched' : 'Type mismatch' });

  // Performance
  const waitStatus = waitH < 3 ? 'pass' : waitH < 6 ? 'ok' : 'tight';
  checks.push({ category: 'Performance', parameter: 'Wait Time', status: waitStatus, icon: waitStatus === 'tight' ? '⚠' : '✔', detail: `Historical average wait: ${waitH.toFixed(1)}h ${waitH < 3 ? '— excellent availability' : waitH < 6 ? '— acceptable' : '— above average'}`, compact: `Wait ${waitH.toFixed(1)}h` });

  checks.push({ category: 'Performance', parameter: 'Service Time', status: 'info', icon: 'ℹ', detail: `Historical average service: ${serviceH}h (derived from past operations, not used in ranking)`, compact: `Service ${serviceH}h` });

  // Commercial
  checks.push({ category: 'Commercial Impact', parameter: 'Cost Estimate', status: 'info', icon: 'ℹ', detail: `Estimated port charges based on vessel size and berth tariff category`, compact: 'Standard tariff' });
