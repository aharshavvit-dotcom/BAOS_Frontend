...
def _get_port_code(session, port_name: str) -> str:
    ...
    from backend.database.baos_models import BaosPort
...
def load_port_config(port_name: str) -> dict:
    ...
    try:
        from backend.database.connection import get_sync_db
        from backend.database.baos_models import BaosPort, BaosBerth, BaosPortCall, BaosAssumptionConfig
...
def load_history(port_name: str) -> pd.DataFrame:
    ...
    try:
        from backend.database.connection import get_sync_db
        from backend.database.baos_models import BaosPort, BaosPortCall
...
def is_trained(port_name: str) -> bool:
    ...
    try:
        from backend.database.connection import get_sync_db
        from backend.database.baos_models import BaosPort, BaosMLModelRegistry
        session = get_sync_db()
        try:
            port_code = _get_port_code(session, port_name)
            port = session.query(BaosPort).filter(BaosPort.port_code == port_code).first()
