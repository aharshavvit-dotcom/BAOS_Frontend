...
test_cases = [
    {
        "id": "30684004", "name": "BOURBON", "vessel_type": "Chemical", "cargo_type": "Chemical",
        "loa": 183.20, "draft": 9.50, "beam": 32.224, "dwt": 46921, "assigned_berth": "5355", "assigned_name": "Berth BD1"
    },
    {
        "id": "30493410", "name": "BOURBON", "vessel_type": "Chemical", "cargo_type": "Chemical",
        "loa": 183.20, "draft": 10.20, "beam": 32.224, "dwt": 46921, "assigned_berth": "5355", "assigned_name": "Berth BD1"
    },
    {
        "id": "29591751", "name": "AHINOS", "vessel_type": "Chemical", "cargo_type": "Chemical",
        "loa": 183.20, "draft": 10.80, "beam": 32.233, "dwt": 53187, "assigned_berth": "4322", "assigned_name": "Berth BD2"
    },
    {
        "id": "30354621", "name": "ASIA EVERGREEN", "vessel_type": "Chemical", "cargo_type": "Chemical",
        "loa": 146.00, "draft": 7.50, "beam": 22.0, "dwt": 14000, "assigned_berth": "4322", "assigned_name": "Berth BD2"
    },
    {
        "id": "28401474", "name": "ANGIE", "vessel_type": "Chemical", "cargo_type": "Chemical",
        "loa": 183.20, "draft": 11.70, "beam": 32.233, "dwt": 52420, "assigned_berth": "4322", "assigned_name": "Berth BD2"
    },
    {
        "id": "30520011", "name": "GLOBAL JUPITER", "vessel_type": "Chemical", "cargo_type": "Chemical",
...
print("Running test cases against recommender...", flush=True)
print("-" * 80, flush=True)
for tc in test_cases:
    vessel = {
        ...
        "cargo_type": tc["cargo_type"],
    }
    options = recommend_berth(vessel, "chennai", top_k=5)
    print(f"Vessel: {tc['name']} ({tc['vessel_type']}, LOA: {tc['loa']}, Draft: {tc['draft']})", flush=True)
    print(f"Target: {tc['assigned_name']} ({tc['assigned_berth']})", flush=True)
    target_rank = -1
    for opt in options:
        marker = " <---------- TARGET" if tc['assigned_berth'] == opt.berth_code else ""
        if tc['assigned_berth'] == opt.berth_code:
            target_rank = opt.rank
        print(f"  Rank {opt.rank}: {opt.berth_name} ({opt.berth_code}) - Conf: {opt.confidence:.1f}% Suit: {opt.suitability_score:.1f}%{marker}", flush=True)
    
    if target_rank == -1:
        print(f"  --> TARGET NOT IN TOP 5", flush=True)
